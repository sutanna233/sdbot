...
"""
    ...
    def _agent_system_prompt(self):
        ...
        return (
            ...
            "- prompt choices 必须参考“苏丹娜记忆中的你”的长期偏好；但用户当前明确要求优先级最高，例如用户说 SFW 就不得被长期 NSFW 偏好覆盖。\n"
            "- prompt choices 的差异应围绕构图、姿态、表情、服装状态、场景、镜头重点和强度；不要以画风作为主要差异，画风由 artist tags / artist combo 控制。\n"
            "- prompt choices 的 label 不要写成“xx风/xx画风/xx风格”。\n"
            "- prompt choices 的 dream.description 只写最终描述，不要包含推理、问号、不确定标记、箭头改写或“X -> Y”这类纠错痕迹。\n"
            "- 如果用户指定角色、服装、动作、LoRA、artist tags、尺寸或模式，所有 prompt choices 都必须保留这些约束。\n"
