...
- **WebUI**：提供浏览器控制台，用于聊天、生成、配置、LoRA、技能、统计和日志查看。
- **守护进程模式**：`daemon` 命令一键启动 WebUI + Telegram，支持 systemd 开机自启。
- **日志系统**：集中式日志记录，按大小轮转，支持 WebUI 在线查看和 Agent 自然语言查询。
...
```bash
pip install pipx          # 如果还没装
pipx install -e /vol1/sdbot
```

装完后 `sdbot` 命令全局可用，无需激活虚拟环境。
...
python3 -m venv .venv
source .venv/bin/activate
pip install -e .          # 注册 sdbot 系统命令
```

之后在任何目录可直接运行 `sdbot shell`。源文件改动后无需重装。
...
2. 运行 `python sdbot.py shell`。
3. 没有 `config.yaml` 会自动进入配置助手，按提示填写 SD 地址和 LLM API Key。
4. 输入自然语言绘图请求，例如：
...
| 方式 | 命令 |
|------|------|
| Agent CLI | `python sdbot.py shell` 或 `run.bat` (Windows) |
| 配置助手 | `python sdbot.py setup` |
| 后台守护（WebUI + Telegram） | `python sdbot.py daemon` |
| WebUI | `python sdbot.py webui --host 127.0.0.1 --port 7861` |
| Telegram Bot | `python sdbot.py telegram start` |
| 自然语言出图 | `python sdbot.py dream "画一个白发异色瞳的猫娘"` |
| 批量 artist combo | `python sdbot.py run --mode combo --num 20` |
| 查看日志 | `python sdbot.py shell` 中输入「查看日志」 |
| 安装 systemd 服务 | `sudo bash deploy/install-service.sh` |
| 重新配置 | `python sdbot.py setup` |

> 如果用 pipx 或 `pip install -e .` 安装，把 `python sdbot.py` 替换为 `sdbot` 即可。（如 `sdbot shell`）
...
| 命令 | 说明 | 示例 |
| --- | --- | --- |
| `setup` | 首次启动配置助手 | `python sdbot.py setup` |
...
Agent 会根据意图调用工具。通用绘图请求会先给出 prompt choices，选择后再执行 `dream`；包含具体角色、人物或作品名的绘图请求会先用 `character_resolve` 做本地 alias / tag cache 解析。未命中时，系统会让 LLM 只提出 Danbooru tag 候选，再用 tagsite/cache 验证候选，只有高置信解析后才会基于结构化标签生成 choices；低置信或同名角色会要求用户确认候选，或选择按原描述直出，避免未确认就脑补角色设定。生成完成后会保存上一张图的结构化信息，因此“完整提示词”“这张”“上一张”“输出目录”等追问可以复用当前会话上下文。

## 守护进程部署（Linux）

将 sdbot 安装为 systemd 服务，开机自启、崩溃自动重启：
...
| 命令 | 说明 |
|------|------|
| `systemctl status sdbot-daemon` | 查看服务状态 |
| `journalctl -u sdbot-daemon -f` | 查看实时日志 |
| `systemctl restart sdbot-daemon` | 重启服务 |
| `systemctl stop sdbot-daemon` | 停止服务 |

服务默认同时启动 WebUI（127.0.0.1:7861）和 Telegram Bot（需配置 token）。

## 日志系统

sdbot 内置集中式日志，默认写入 `logs/sdbot.log`，按 10MB 大小轮转，保留 30 份。

**在 WebUI 中查看：** 访问 http://127.0.0.1:7861/logs，支持级别筛选、关键词搜索、自动刷新。

**通过 Agent 查询：** 在 shell 或 Telegram 中输入「查看日志」「最近有什么报错」「搜一下 SD API」即可。
